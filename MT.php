...
<center><img src="LSB-logo.png" alt="" height="100px"></center>
            ...
            <table>
                <tr>
                    ...
                    <td><?php include_once "db.php";
                                $db=dcon();
                                $qr=$db->query("select * from lena where Username = '{$_SESSION['username']}'");?>
                                <select name="from" id="">
                                <?php while ($res=$qr->fetch_assoc()) { ?>
                                <option style="font-size: 15px;" value="<?php echo $res['Account'];?>"> <?php echo $res['Account'];?> </option><?php } ?>
                                </select>
                        <?php $db->close(); ?>
                    </td>
                </tr>
    ...
    if($from == 0)
    {
        echo "<br><b><big><strong>From Account Number is Invalid</strong></big></b>";
    }
    else if($to == 0)
    {
        echo "<br><b><big><strong>To Account Number is Invalid</strong></big></b>";
    }
    else
    {
