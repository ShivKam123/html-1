...
<body>
    ...
    <center>
        ...
        // echo "Amount = $amount  ,  Deposit = $deposit";
        $amount = $amount+$_REQUEST['amount'];
        $deposit = $deposit+$_REQUEST['amount'];
        // echo "Amount = $amount  ,  Deposit = $deposit";
        $qr = $m->query("update lena set Amount = $amount , Deposit = $deposit WHERE Account = '{$_SESSION['account']}'");
        $date = date("d/M/y");
        $time = date("h:m:s");
        $qr = $m->query("insert into tab1(Account,Withdrawal,Deposit,Time,Date) values('{$_SESSION['account']}','0','{$_REQUEST['amount']}','$time','$date')");
        $m->close();
        echo "<b><big><strong>Amount has been deposited: {$_REQUEST['amount']}</strong></big></b><br>";
        echo "<b><big><strong>Balance: $amount</strong></big></b>";
        ?>
    </center>
    <hr><div class="back"><a href="Edeposit.php"><b><big><strong>Back</strong></big></b></a></div>
    </div>
</body>
</html>